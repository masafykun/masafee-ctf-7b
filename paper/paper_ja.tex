...
\begin{itemize}
\item \textbf{Style transfer は発生する}: 学習済みモデルは CTF writeup の形式・用語・直接的な表現を獲得する．
\item \textbf{Capability transfer は本規模では発生しない}: 知識多肢選択テストおよび実 CTF 問題上で，学習モデルはベースモデルと統計的に区別不能か，あるいは弱い．
\item \textbf{Style overfitting はタスク性能を能動的に低下させ得る}: 学習モデルは writeup 形式のナラティブ出力にコミットし，応答トークン上限を最終回答が出る前に消費する．
\item \textbf{SOC 特化リファレンスとのスタイル乖離は大きく定量的に測定可能}: Cisco の Foundation-Sec-8B-Instruct \cite{cisco2025foundationsec} と比較して，本モデルは hedging 表現を約 11 倍少なく使用する．これは学習データ領域 (CTF writeup vs SOC analysis テキスト) を反映したものであり品質差ではない．hedging 表現は SOC コンテキストでは機能的に適切である．
\end{itemize}
...
最大の知見は，生 CTF writeup 上の継続事前学習が，我々が試した 7B 規模では style transfer 操作として作用するということである．モデルは認識可能な writeup 形式と CTF 固有用語を獲得する．知識 MCQ 上でも single-shot CTF 問題上でもベースを超える測定可能な capability を獲得しない；後者のタスクでは獲得したスタイルプライアが能動的に有害である．

寄与要因として 2 つを提案する．第一に，18{,}013 件 writeup チャンク (10.6\,M トークン) は Qwen 2.5 Coder の事前学習コーパスに比べ小さく，ベースの推論能力を超える押し上げは起きにくい．第二に，writeup は事後ナラティブで，表面形式が極めて特徴的だが内部論理は不均一 (問題ごと・ツールごとに異なる)；次トークン確率を最大化するよう学習されるモデルは，最も予測しやすい表面パターンに容量を合理的に割り当てる．
...
\item 直接回答タスクの capability transfer が目的なら，事後ナラティブテキスト上の生継続事前学習は，同コーパス由来のキュレートされた $\langle\text{プロンプト}, \text{回答}\rangle$ ペア上の instruction tuning に劣る可能性が高い．
...
我々は \textbf{masafee-ctf-7b} を作成し公開した．これは Qwen 2.5 Coder 7B Instruct を CTF writeup テキストで QLoRA 継続事前学習した 7B パラメータ微調整モデルで，単一消費者向け GPU 上で 12 時間 17 分・電気代約 150 円で学習された．実証的に，微調整は測定可能な style transfer (CTF writeup 形式・用語・直接性) を生むが，知識 MCQ ベンチマーク上でも single-shot CTF チャレンジ攻略上でも測定可能な capability transfer を生まない；実際後者のタスクでは獲得したスタイルプライアが能動的に有害である．得られたモデルが Cisco の Foundation-Sec-8B-Instruct と同一 CTF プロンプトで約 11 倍少ない hedging 表現を産出することも報告した．これは品質順序ではなく学習データ領域を反映している．
...
本研究が依存するオープンソースツール (\texttt{unsloth}, \texttt{transformers}, \texttt{peft}, \texttt{llama.cpp}, Ollama) のメンテナ；再現可能なベンチマークを公開した CyberMetric および NYU CTF Bench の著者；学習コーパスを構成する CTFtime writeup の寄稿者；公開リファレンスとして Foundation-Sec-8B をリリースした Cisco fdtn-ai チームに感謝する．
