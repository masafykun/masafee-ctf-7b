...
We make the following observations within the scope of the experimental setup described in this paper:
...
\item \textbf{Capability transfer is not observed in our setting}. On the knowledge multiple-choice tests and practical CTF challenges used in this work, the fine-tuned model is statistically indistinguishable from or weaker than the base model.
...
\subsection{Style Transfer vs.\ Capability Transfer in This Setting}

Our headline finding is that continued pretraining on raw CTF writeups acts as a style-transfer operation under the setting we tested (7B base, $\sim$10\,M-token corpus, raw-text continued pretraining, single-shot evaluation). The model acquires recognisable writeup formatting and CTF-specific terminology. It does not acquire measurable capability beyond the base model on the knowledge MCQ benchmark and single-shot CTF problem set used in this work; on the latter task its acquired stylistic prior is actively detrimental. We do not claim this generalises to larger corpora, instruction-format data, larger model scales, or agentic protocols.
...
We have produced and released \textbf{masafee-ctf-7b}, a 7B-parameter QLoRA continued-pretraining fine-tune of Qwen 2.5 Coder 7B Instruct on CTF writeup text, trained in 12 hours 17 minutes on a single consumer GPU at an electricity cost of approximately 150\,JPY. \textbf{Within the scope of this experimental setup}, the fine-tune produces measurable style transfer (CTF writeup formatting, terminology, directness) but no measurable capability transfer on the knowledge MCQ benchmark and single-shot CTF challenge set used in this work; in fact the acquired style prior is actively detrimental on the latter task. We have also reported that the resulting model emits approximately 11$\times$ less hedging-language than Cisco's Foundation-Sec-8B-Instruct on identical CTF prompts, reflecting their respective training-data domains rather than a quality ranking.

Our findings should be read as a characterisation of \emph{this} configuration (7B Qwen 2.5 Coder base, $\sim$10\,M-token raw-text continued pretraining, single-shot evaluation), not as a general claim about continued pretraining. Larger corpora, instruction-format data, larger model scales, or agentic evaluation protocols may produce different outcomes.
